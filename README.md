...
For the deployed candidate screening workflow (including **Resume Screening**, **Screening History**, **Real-Time Prediction**, and **Candidate Ranking**), Logistic Regression is used consistently. This ensures that predictions, fairness thresholds, and SHAP explanations remain consistent across the application. The **AI Resume Screening & Persistence** module processes single and bulk resume uploads deterministically, auto-saves screening audit trails to persistent JSON history (`data/resume_screening_history.json`), tracks candidate statuses (`Pending`, `Shortlisted`, `Rejected`), and enables re-opening complete historical SHAP explanations without altering core training artifacts or fairness benchmarks.
